...
class OwnerMessageModel {
  // Get all customers that have bookings with the boat owner
  static async getCustomersForOwner(ownerId) {
    try {
      const query = `
        SELECT DISTINCT 
          u.user_id,
        ...
        WHERE b.owner_id = ?
        GROUP BY u.user_id, u.full_name, u.profile_pic, u.email, u.phone_number, b.boat_id, b.name, bk.booking_id
  ...
  static async getMessages(ownerId, customerId, bookingId = null) {
    try {
      ...
      `;
      const params = [ownerId, customerId, customerId, ownerId];
  ...
  static async sendMessage(senderId, receiverId, message, bookingId = null) {
    try {
      ...
      console.log('Saving owner message with bookingId:', bookingId, 'for sender:', senderId, 'receiver:', receiverId);
  ...
  // Mark messages as read for boat owner
  ...
  static async getUnreadCount(ownerId, bookingId = null) {
    try {
      let query = `
  ...
  static async getRecentConversations(ownerId) {
    try {
      const query = `
        ...
        LEFT JOIN boats b ON b.owner_id = ?
      ...
      `;
      const [rows] = await db.execute(query, [ownerId, ownerId, ownerId, ownerId, ownerId, ownerId]);
      return rows;
    } catch (error) {
      throw error;
    }
...
module.exports = OwnerMessageModel;
